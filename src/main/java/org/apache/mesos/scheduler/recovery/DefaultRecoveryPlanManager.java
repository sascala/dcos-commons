...
public class DefaultRecoveryPlanManager extends ChainedObserver implements PlanManager {
     ...
     * 2. If the task needs recovery, removes any COMPLETED blocks for this task
     *    (at most one block for a given task can exist).
     * 3. If the task needs recovery, creates a new PENDING block.
     *
    ...
    public void update(Protos.TaskStatus status) {
        try {
            ...
            if (!taskInfo.isPresent()) {
                LOGGER.error("Failed to fetch TaskInfo for task (name=%s)".format(taskName));
                return;
            }
            ...
            LOGGER.error("Error updating status for task (id=%s)".format(status.getTaskId().toString()), ex);
        }
    }
