...
public class DefaultSchedulerTest {
    ...
    public void testLaunchAndRecovery() throws Exception {
        ...
        verify(mockSchedulerDriver, timeout(1000).times(1)).acceptOffers(
                (Collection<Protos.OfferID>) Matchers.argThat(contains(offer1.getId())),
        ...
        Protos.Offer offer2 = Protos.Offer.newBuilder(getSufficientOfferForTaskB())
        ...
        defaultScheduler.resourceOffers(mockSchedulerDriver, Arrays.asList(offer2));
        defaultScheduler.awaitTermination();
    ...
    private boolean inExpectedState(Plan plan, List<Status> blockStatuses) {
        if (countBlocks(plan) != blockStatuses.size()) {
            return false;
        }
    ...
    private List<Protos.TaskID> install() {
        ...
        Plan plan = defaultScheduler.getPlan();
        Assert.assertTrue(inExpectedState(plan, Arrays.asList(Status.COMPLETE, Status.COMPLETE, Status.COMPLETE)));
